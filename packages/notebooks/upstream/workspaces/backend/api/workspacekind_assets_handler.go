...
func (a *App) getWorkspaceKindAssetHandler(w http.ResponseWriter, r *http.Request, ps httprouter.Params, assetType wskAssetType) {
	...
	if namespace != "" {
		...
		authPolicies = []*auth.ResourcePolicy{
			auth.NewResourcePolicy(auth.ResourceVerbList, &kubefloworgv1beta1.Workspace{
		...
		authPolicies = []*auth.ResourcePolicy{
			auth.NewResourcePolicy(auth.ResourceVerbGet, &kubefloworgv1beta1.WorkspaceKind{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "kubeflow.org/v1beta1",
					Kind:       "WorkspaceKind",
				},
				ObjectMeta: metav1.ObjectMeta{Name: wskName},
			}),
		}
	}
